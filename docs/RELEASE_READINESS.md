...
9. **Transactional email prerequisites** (ADR-P026 / FEATURE-011) — **PARTLY
   ...
   **Still outstanding:** (e) native **Universal / App Links still need a native
   ...
   verification) has no runtime** — V2-A defines schema only — so it retains
   every prerequisite above in its own right. **No email is ever sent from
