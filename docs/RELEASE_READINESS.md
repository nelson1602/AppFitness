...
Last updated: 2026-09-04 (email-verification **Production** gate) · Current
main baseline `efd75ff9` · App version `1.0.0` · Publication date: **not set**.
...
> **2026-09-04 — email verification is Production-validated.** Both V2-E halves
> passed, Development first and then the separately authorized Production gate.
> Each environment now has its own account portal — `account.appfitnessrd.com`
> for Production, `account-dev.appfitnessrd.com` for Development — with disjoint
> CORS allow-lists, and a real Spanish message was delivered, redeemed, replayed
> and cleaned up against a synthetic **Production** account. The **Email
> verification** gate below moves **NOT MET → PASS**. **Nothing else moves** —
> deep-link completion, physical-device validation, Play Console, Production
> environment backup/PITR, rollback dry-run, accessibility and legal sign-off
> are untouched, and this is **not** a statement of publication readiness.
...
| **Email verification available (ADR-P026, FEATURE-011 Vertical 2)** | **PASS** — Production-validated 2026-09-04 | Added as a V1 launch requirement by **ADR-P026 (2026-08-27)**; also resolves **TECHDEBT-001**'s requirement that the verification strategy be explicitly decided. Audited at `f1e7214`: `User` has **no `emailVerified`/`emailVerifiedAt`**, no verification-token model, no verify/resend endpoint, and no mobile route or copy. Requires: `emailVerifiedAt` column + verification-token migration (same storage design, single-use, **24-h TTL**), `resend-verification` / `verify-email` endpoints, persistent soft-gate reminder, and legacy backfill. **Policy:** existing accounts are backfilled `emailVerifiedAt = NULL` and are **never locked out**; **no `PENDING_VERIFICATION` `UserStatus`**; unverified users keep **core access**, with verification mandatory only before future email-report / account-notification features. **V2-A (2026-09-03) delivers the schema only** — nullable `users.email_verified_at`, the `email_verification_tokens` table (hash-only unique; a partial unique index on `user_id` `WHERE consumed_at IS NULL AND invalidated_at IS NULL` — i.e. **one open unconsumed/uninvalidated row per user**, expiry deliberately not in the predicate; cascade on user delete), and the two `EMAIL_VERIFICATION_*` `AuditAction` values, in two additive forward-only migrations validated against disposable PostgreSQL. **V2-C (2026-09-03) delivers the backend** — automatic issuance at registration (best-effort; a mail failure never alters or rolls back the registration response), the **authenticated** `POST /auth/resend-verification` (no email field; same generic `202` for every accepted request — dispatched, already verified, mail failure, per-account-ceiling no-op — with `400`/`401`/`429`/`503` at the boundaries, per ADR-P026 §Clarifications 2026-09-03), the **public** `POST /auth/verify-email` (204, single-use, generic 400, **creates no session**), the EN/ES mail template, 24-hour SHA-256-only tokens, per-IP throttles plus a per-account ceiling, both `EMAIL_VERIFICATION_*` audit actions, Sentry scrubbing, and `emailVerifiedAt` on `GET /auth/me`. Proved against **disposable PostgreSQL only** (22 e2e cases incl. concurrent redemption, concurrent resend, replay, expiry, expired-but-open replacement, throttling and mail-failure independence); **CI sends no email by construction** (`FakeMailTransport`). **V2-D (2026-09-03) delivers the user-facing surface** — the `/verify-email` Expo Router route shared by native and Web (fragment capture after hydration, token scrubbed from the URL and history, frozen loading/success/invalid/incomplete/request-failed states, **creates no session**, conditional Continue/Go-to-sign-in navigation), the advisory dashboard reminder shown only while `emailVerifiedAt` is null with resend (no email field) and an **in-memory, current-session-only** dismissal, and all **23** `auth.verify.*` keys in **EN and ES with exact parity**. Soft gate preserved: unverified users keep full core access. Validated with type-check, lint, format, **1320 mobile unit tests across 151 suites with coverage thresholds met**, Expo Doctor 21/21, and iOS/Android/Web exports (`/verify-email` prerenders). **V2-E — BOTH HALVES PASSED 2026-09-04.** *Development half (synthetic account):* the then-Development host returned **200**, Development CORS allowed **exactly** that origin, `MAIL_VERIFICATION_BASE_URL` was set to it, and Development deployment `6faf550d…` reached **SUCCESS** on merge `5b09f5ae`. *Production half (separately authorized, synthetic Production account):* each environment ended on its **own** portal, served by its own Cloudflare Worker version from its own single-environment bundle — **Development** `https://account-dev.appfitnessrd.com`, Worker `e4e4308d-a655-4f16-9de4-40819195a394`, after Development Railway deployment `7ceecdec-9d53-4a3c-80bc-da5b95bf0f09` succeeded on moving `MAIL_VERIFICATION_BASE_URL` and CORS to that hostname; **Production** `https://account.appfitnessrd.com`, Worker `bcc9ed68-e25d-406d-8618-f1618108bd0e`, after Production Railway deployment `bd236717-410d-43ec-8dfa-635c3ab01972` succeeded on enabling `MAIL_VERIFICATION_BASE_URL` and adding the **exact** Account origin to CORS **while preserving Recovery**. **Both `/health` endpoints returned HTTP 200**, and the allow-lists are **disjoint**: Production accepts only its Account/Recovery origins and **rejects the Development Account origin**; Development accepts its Account-Dev origin and **rejects the Production Account origin** — verifying that **each browser portal is prevented by CORS from calling the other environment's API**. Cross-environment token redemption was **not** tested and is **not** claimed. A **real synthetic Production registration** delivered the **Spanish** transactional verification email with **open/link tracking disabled** and the token **exclusively in the URL fragment**. Issuance stays **best-effort**: Production now **attempts** verification delivery for registrations, and a mail failure remains **non-blocking** with resend available. The **first opening** returned the empty **204**, rendered **success**, contacted **only the Production API**, **scrubbed the fragment**, and created **no cookie and no session**. **Reopening the original link in a fresh browser realm** again returned the empty **204** and rendered **success**, contacting only the Production API, scrubbing the fragment, and creating **no cookie and no session**. Database state was **not** compared before and after the replay, so **no no-mutation claim rests on this gate** — that invariant is established by **ADR-P029 and its automated tests**, not by this external validation. Authenticated `GET /auth/me` confirmed `emailVerifiedAt`; the synthetic account was **deleted** and a subsequent login returned **401**. **Scope:** closes **Vertical 2 only**. **Deep-link completion (Universal / App Links + native rebuild) remains a separate open V1 gate** — an emailed link still opens the Web portal, not the app. No address, credential, token, raw link, Postmark message or request identifier, or secret is recorded. |
...
9. **Transactional email prerequisites** (ADR-P026 / FEATURE-011) — **PARTLY
   ...
   **Vertical 2 (email verification) is Production-validated 2026-09-04**, on its
   own Account host with a CORS allow-list disjoint from Development's, so it now
   holds every prerequisite above in its own right.
   **Still outstanding:** (e) native **Universal / App Links still need a native
   rebuild** — unchanged and **not** claimed complete, so an emailed link opens
   the Web portal rather than the app. **No email is ever sent from
