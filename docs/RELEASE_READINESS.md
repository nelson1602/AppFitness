...
Last updated: 2026-09-04 (email-verification **Development** gate) · Current
main baseline `5b09f5ae` · App version `1.0.0` · Publication date: **not set**.
...
> **2026-09-04 — email verification passed its Development gate only.** The
> account host resolves, Development `MAIL_VERIFICATION_BASE_URL` points at it,
> and a real Spanish message was delivered, redeemed, replayed and cleaned up
> against a synthetic account. **The Email verification gate below stays NOT
> MET**: it is a Production gate, and **Production activation requires separate
> explicit authorization and its own evidence**. **Nothing else moves** —
> deep-link completion, physical-device validation, Play Console, rollback
> dry-run and legal sign-off are untouched, and this is **not** a statement of
...
9. **Transactional email prerequisites** (ADR-P026 / FEATURE-011) — **PARTLY
   ...
   **Still outstanding:** (e) native **Universal / App Links still need a native
   ...
   2026-09-04, but sends nothing in Production** — its account host is live and
   Development delivery works, while **Production activation is a separate
   authorized gate that has not been run**. **No email is ever sent from
