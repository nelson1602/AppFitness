...
9. **Transactional email prerequisites** (ADR-P026 / FEATURE-011) — **PARTLY
   ...
   **Still outstanding:** (e) native **Universal / App Links still need a native
   ...
   verification) is built end to end (V2-A/C/D) but sends nothing** — V2-E
   remains — so it retains every prerequisite above in its own right, plus the
   account host its links point at. **No email is ever sent from
