...
// (FDC-sourced grams), brown_rice an ADR-P013 Batch-3A cup-food anchor, and
// broccoli an ADR-P013 Batch-3B cup-vegetable anchor.
const GOLDEN = [
  {
...
describe('serving snapshot derivation (server)', () => {
  it('derives the immutable per-serving snapshot for a known food', () => {
    ...
    expect(deriveServingSnapshot(chicken)).toEqual({
      ...
      catalogVersionSnapshot: 'food-catalog@1.5.0',
