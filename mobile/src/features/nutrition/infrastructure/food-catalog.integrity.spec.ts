...
describe('food catalog integrity', () => {
  ...
  it('records an authored gram weight only on known non-gram servings (no fabrication)', () => {
    ...
    // the 13 ADR-P013 Batch-2 `tbsp` foods, the 4 tsp-semantics `tsp` foods,
    // and the 26 ADR-P013 Batch-3A `cup` grains/legumes/staples. FDC-sourced full-serving weights
    ...
    expect(withGrams.filter((f) => f.servingSize.unit === 'cup')).toHaveLength(26);
  });
