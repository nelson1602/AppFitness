...
describe('food catalog integrity', () => {
  ...
  it('records an authored gram weight only on known non-gram servings (no fabrication)', () => {
    ...
    // 26 ADR-P013 Batch-3A `cup` grains/legumes/staples, and the 42 ADR-P013
    // Batch-3B `cup` vegetables. FDC-sourced full-serving weights
    ...
    expect(withGrams.filter((f) => f.servingSize.unit === 'cup')).toHaveLength(68);
  });
