...
| `food-log-exclusion-warning.yml` | yes (no seed) | **PENDING run** — add a "Nuts" allergy via the dietary-preferences surface, then confirm selecting a nut food on the food log shows the non-blocking allergy/sensitivity warning and can still be logged (ADR-P014 Slice 4). Not wired into CI yet: it needs a **new EAS `e2e` APK** containing the Slice 4 warning UI (a build gate, out of scope for the slice). Until then, Slice 4 is verified by unit/component tests. |
