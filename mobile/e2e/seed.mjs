...
//   E2E_SEED_SCOPE  'full' (default) seeds profile + evaluation + goal for
//                   the seeded-pull flow (dashboard-sync.yml). 'evaluation'
//                   seeds ONLY a weight evaluation — used by the device-side
//                   onboarding-loop flow, where the profile and goal are
//                   entered on the device and only the weight measurement
//                   (which has no on-device entry UI yet, TEST-004) must be
//                   supplied server-side so the iCoach assessment can become
//                   ready. Seeding evaluation-only avoids creating a server
//                   profile/goal that would conflict with the device entries.
...
const SCOPE = process.env.E2E_SEED_SCOPE ?? 'full';

if (SCOPE !== 'full' && SCOPE !== 'evaluation') {
  throw new Error(`E2E_SEED_SCOPE must be 'full' or 'evaluation', got '${SCOPE}'`);
}
...
console.log(`[seed] logged in as ${EMAIL} (scope=${SCOPE})`);
...
// the public sync contract (doubles as a second-device sync test). Only
// for the full seeded-pull flow; the onboarding-loop flow sets its goal
// on the device.
if (SCOPE === 'full') {
...
console.log(`[seed] done (scope=${SCOPE})`);
