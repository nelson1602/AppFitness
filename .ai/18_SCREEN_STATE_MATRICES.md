...
| **Data-gap** | `status === 'empty'` — the read succeeded but `data.assessment` is `null` because prerequisites are missing | `<DataGapCard gaps={data.missing} />`, card-local, one named item per gap with a routing button | User supplies the named input on the owning screen | Both | `DashboardScreen.tsx:81-90`; `dashboard.store.ts:19`; spec *"renders the empty state and dev sample action"*; `dashboard.gap.*` (19 keys) | SHIPPED |
